...
% close all; clear all
function [rAnchor, L_moor, fMoor0, mMoor0, cMoor0]  = solveStaticMooring(dPtfm, draft)
...
zFairlead = -abs(draft);          % vertical Fairlead attachment
...
for iL = 1:length(L0)
    ...
    if iL<10
        figure(1)
        subplot(3,3,iL)
        plot(dspSurge*200,-Fmoor(1,:,iL),[0 dspSurge(end)*200],[F_wind F_wind],'linewidth',2);hold on;
        xlabel('xSurge [m/200]'); ylabel('Force H [N]');title(['line length',num2str(L)]);% xSurge is ploted in full scaled
        
        figure(2)
        subplot(3,3,iL)
        plot(dspSurge*200,-Fmoor(3,:,iL),'linewidth',2);hold on;
