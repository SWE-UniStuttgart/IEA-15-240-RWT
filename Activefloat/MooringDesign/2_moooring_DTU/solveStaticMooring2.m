...
% close all; clear all
function [rAnchor, rFairlead, ra, rf, L_moor, fMoor0, mMoor0, cMoor0]  = solveStaticMooring(dPtfm, draft)
...
rFairlead = dPtfm+D_TP;   % radius of fairleads [m]
...
rAnchor = 2/sind(60);  %max                    % radius of anchor
Lmin = sqrt((rAnchor-rFairlead)^2+(zFairlead-zAnchor)^2);
Lmax = (rAnchor-rFairlead)+abs(zFairlead-zAnchor);
L0 = (Lmin+0.15*(Lmax-Lmin)):0.01:(Lmax-0.5*(Lmax-Lmin));  % need to played around, too long or too short
            ...
            dspSurgeRated(iL) = interp1(-Fmoor(1,:,iL),dspSurge,F_wind,'linear','extrap');
             ...
             if iL > 2
                dspSurgeRated(iL)= dspSurgeRated(iL-1)*2-dspSurgeRated(iL-2);
                 ...
                 dspSurgeRated(iL)= dspSurgeRated(iL-1)*2;
            % else
             %    dspSurgeRated(iL) = 0;
             end
         end
